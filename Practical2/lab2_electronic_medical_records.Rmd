...
- **note_id: a unique integer from 1 to 4999 for each case.**
- **description: a summary of the case.**
- **medical_speciality: The department that is responisble for the case report.**
- **sample_name: The title of the sample procedure.**
- **transaction: a description that starts with "clinical note" of the case.**
- **keywords: The words in the report including medical departments and sample names.**
...
tokenized.data.unigram %>%
  dplyr::distinct(medical_specialty, word) %>%
...
```{r}
tokenized.data %>%
  dplyr::distinct(medical_specialty, ngram) %>%
  dplyr::group_by(medical_specialty) %>%
  dplyr::summarise(unique_unigrams = dplyr::n())
```
...
analysis.data %>%
  ...
  dplyr::distinct(medical_specialty, sentence) %>%
  dplyr::group_by(medical_specialty) %>%
  dplyr::summarise(unique_unigrams = dplyr::n())
```
...
- **Not without cleaning the text and fine tuning the lemmatizer, because the general purpose lemmatizer usually does not recognize the specialized words, and medical terms usually missed lemmatizaer dictioneries and can be overlooked. And some medical words have a different meaning in a medical test than a daily use.**
...
- **It shows that the lemma frequencies between the two plots (Radiology and Surgery) are not the same, which means their terms are on different rates.For example Radiology uses imaging terms like (series images) while Surgery uses prcedural terms (Carpal ligament), so because one plot (Radiology) focuses on imaging language and the other one (Surgery) focuses on procedures, they are expected to have different terms.**
...
- **Yes, the very specific speciality wise TF-IDF lemma at the top 10 (Orhtopedic - range motion, carpa ligament .. ets). In Radiology (myocardial perfusion, motor units, ..), and in (Syrgery - anterior chamber, lithotomy position, .. etc)**
...
```{r}
analysis.data %>% dplyr::select(medical_specialty, transcription) %>% dplyr::filter(stringr::str_detect(transcription, 'range motion')) %>% dplyr::slice(1)
```
...
#use LDA function to fit a 6 topic (`k=6`) LDA-model
```{r}
...
- **Yes, the 3 topic LDA still turn up when you run it with 6 topics, and the specialties get splits into subtopics (Orthopedric - "range motion" shown in topics 1 and 2), (Radilogy - "coronay artery" shown in topics 3 and 6) and (Surgery - "blood loss" shown in topics 4 and 5).**
